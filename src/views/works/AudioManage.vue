            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    <el-popover trigger="hover" placement="left">
                        <div align="center">
                            <audio controls v-bind:src="scope.row.url" style='width: 250px; height: 50px; padding: 0px 0px; align-content: center' />
                        </div>
                        <p>{{scope.row.title}}</p>
                        <p>作者:
...
export default {
    ...
    methods:{
        handleSearch(val) {
            ...
            this.$http.get("http://localhost:8083/works/findByTitle/" + this.currentPage + "/" + this.size+"/"+2+"?keyword="+this.search).then(res=>{
        ...
        getAllData() {
            console.log("create func");
            this.$http.get("http://localhost:8083/works/findAll?currentPage=" + this.currentPage + "&size=" + this.size + "&categoryId=2" ).then(res=>{
                // this.tableData = res.data
                this.tableData = res.data.list
      ...
      handleCurrentChange(val) {
        ...
        handleCheck(option) {
            if (option === 1) {
                ...
                this.$router.push('/homeView/audioManage0');
            }
        },
