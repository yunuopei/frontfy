<template>

    <div>
        <!--      <h1>欢迎{{showUserName}}来到后台管理系统</h1>-->
        <div  class="container mt-5 body">
            <!--          统计1-->
            <div class="row" >
                <div  class="col-md-6  col-lg-3 "   >
                    <el-card style="background-image:linear-gradient(to left,#74ebd5,#acb6e5)" shadow="always">
                        <el-row :gutter="20">
                            <el-col :span="12">
                                <div >
                                    <img src="@/assets/1.png" class="img-fluid" alt="待审核">
                                </div>
                            </el-col>
                            <el-col :span="12">
                                <div >
                                    <div style="font-size: 60px; font-weight: bold; font-family: 'Arial Black'">{{count1}}</div>
                                    <div style="margin-top: 15px">待审核事物数量</div>
                                </div>
                            </el-col>
                        </el-row>
                    </el-card>
                </div>
                <div class="col-md-6  col-lg-3">
                    <el-card style="background-image:linear-gradient(to left,#c0c0aa,#1cefff)" shadow="always">
                        <el-row :gutter="20">
                            <el-col :span="12">
                                <div>
                                    <img src="@/assets/2.png" alt="用户总数" class="img-fluid">
                                    <!--                                  <responsive-image :src="this.imageUrl"/>-->
                                </div>
                            </el-col>
                            <el-col :span="12">
                                <div >
                                    <div style="font-size: 60px; font-weight: bold; font-family: 'Arial Black'">{{count2}}</div>
                                    <div style="margin-top: 15px">平台用户总数</div>
                                </div>
                            </el-col>
                        </el-row>
                    </el-card>
                </div>
                <div class="col-md-6  col-lg-3">
                    <el-card style="background-image: linear-gradient(to left, #c6ffdd,#fbd786,#f7797d);" shadow="always">
                        <el-row :gutter="20">
                            <el-col :span="12">
                                <div>
                                    <img src="@/assets/3.png" alt="非遗项目总数" class="img-fluid">
                                </div>
                            </el-col>
                            <el-col :span="12">
                                <div >
                                    <div style="font-size: 60px; font-weight: bold; font-family: 'Arial Black'">{{count3}}</div>
                                    <div style="margin-top: 15px">非遗项目</div>
                                </div>
                            </el-col>
                        </el-row>
                    </el-card>
                </div>
                <div class="col-md-6  col-lg-3">
                    <el-card style="background-image: linear-gradient(to left,#c2e59c,#64b3f4)" shadow="always">
                        <el-row :gutter="20">
                            <el-col :span="12">
                                <div>
                                    <img src="@/assets/4.png" alt="非遗项目负责人" class="img-fluid">
                                </div>
                            </el-col>
                            <el-col :span="12">
                                <div >
                                    <div style="font-size: 60px; font-weight: bold; font-family: 'Arial Black'">{{count4}}</div>
                                    <div style="margin-top: 15px">非遗项目负责人</div>
                                </div>
                            </el-col>
                        </el-row>
                    </el-card>
                </div>
            </div>
            <div class="row">
                <div  class="col-md-12  col-lg-6 ">
                    <el-divider ></el-divider>
                    <div id="one"></div>
                    <el-divider ></el-divider>
                    <div id="four"></div>
                </div>

                <div class="col-md-12  col-lg-6 ">
                    <div >
                        <el-divider content-position="left">用户统计</el-divider>
                        <div id="two"></div>
                    </div>
                    <div>
                        <el-divider ></el-divider>
                        <div id="three"></div>
                    </div>
                </div>

            </div>

        </div>
    </div>

</template>

<script>
// import ResponsiveImage from 'vue-responsive-image';
const spirit =
    'image://data:image/png;base64,';
// let echarts = require('echarts')
const currentYear = new Date().getFullYear();


export default {


    name: "Welcome",
    data() {
        return {
            maxData:2000,
            imageUrl:"@/assert/2.png",
            count1:'',
            count2:'',
            count3:'',
            count4:'',
            // 图一
            datatwo:[],
            dataone:[],
            datathree:[],
            year:[],

            // 统计图2
            legendData:[],
            seriesData:[],
            name:[],
            value:[],

            //统计图四
            namefour:[],
            valuefour:[],
            datafour:[]
        }
    },

    computed:{
        showUserName()
        {
            return JSON.parse(localStorage.getItem('user'));
        }
    },
    mounted() {
        this.init();
    },
    created() {
        this.count();
    },
    methods:{
        init() {
            setTimeout(() => {
                this.initChartTwo();
                this.initChartOne();
                this.initChartThree();
                this.initChartFour()
            }, 1000);
        },
        initChartOne() {
            const option = {
                title: {
                    text: '项目负责人民族分布情况',
                    left: 'center'
                },
                tooltip: {
                    trigger: 'item',
                    formatter: '{a} <br/>{b} : {c} ({d}%)'
                },
                legend: {
                    type: 'scroll',
                    orient: 'vertical',
                    left:'left',
                    right: 10,
                    top: 20,
                    bottom: 20,
                    data: this.legendData
                },
                series: [
                    {
                        name: '民族',
                        type: 'pie',
                        radius: '55%',
                        center: ['55%', '50%'],
                        data: this.seriesData,
                        emphasis: {
                            itemStyle: {
                                shadowBlur: 10,
                                shadowOffsetX: 0,
                                shadowColor: 'rgba(0, 0, 0, 0.5)'
                            }
                        }
                    }
                ]
            };

            let myChartOne = this.$echarts.init(document.getElementById("one")); //图标初始化\
            myChartOne.setOption(option);
            window.onresize = function () {
                myChartOne.resize();
            };
            myChartOne.on('updateAxisPointer', function (event) {
                const xAxisInfo = event.axesInfo[0];
                if (xAxisInfo) {
                    const dimension = xAxisInfo.value + 1;
                    myChartOne.setOption({
                        series: {
                            id: 'pie',
                            label: {
                                formatter: '{b}: {@[' + dimension + ']} ({d}%)'
                            },
                            encode: {
                                value: dimension,
                                tooltip: dimension
                            }
                        }
                    });
                }
            });
            // 监听容器大小
            window.addEventListener('resize', function () {
                // 调用 resize 和 setOption 方法重新渲染图表
                myChartOne.resize();
                myChartOne.setOption(option);
            });
        },
        initChartTwo() {
            const option = {
                tooltip: {},
                xAxis: {
                    max: this.maxData,
                    splitLine: { show: false },
                    offset: 10,
                    axisLine: {
                        lineStyle: {
                            color: '#999'
                        }
                    },
                    axisLabel: {
                        margin: 10
                    }
                },
                yAxis: {
                    data: this.year,
                    inverse: true,
                    axisTick: { show: false },
                    axisLine: { show: false },
                    axisLabel: {
                        margin: 10,
                        color: '#999',
                        fontSize: 16
                    }
                },
                grid: {
                    top: 'center',
                    height: 200,
                    left: 70,
                    right: 100
                },
                series: [
                    {
                        // current data
                        type: 'pictorialBar',
                        symbol: spirit,
                        symbolRepeat: 'fixed',
                        symbolMargin: '5%',
                        symbolClip: true,
                        symbolSize: 30,
                        symbolBoundingData: this.maxData,
                        data: this.datatwo,
                        markLine: {
                            symbol: 'none',
                            label: {
                                formatter: 'max: {c}',
                                position: 'start'
                            },
                            lineStyle: {
                                color: 'green',
                                type: 'dotted',
                                opacity: 0.2,
                                width: 2
                            },
                            data: [
                                {
                                    type: 'max'
                                }
                            ]
                        },
                        z: 10
                    },
                    {
                        // full data
                        type: 'pictorialBar',
                        itemStyle: {
                            opacity: 0.2
                        },
                        label: {
                            show: true,
                            formatter: function (params) {
                                // return ((params.value / maxData) * 100).toFixed(1) + ' %';
                            },
                            position: 'right',
                            offset: [10, 0],
                            color: 'green',
                            fontSize: 18
                        },
                        animationDuration: 0,
                        symbolRepeat: 'fixed',
                        symbolMargin: '5%',
                        symbol: spirit,
                        symbolSize: 30,
                        symbolBoundingData: this.maxData,
                        data: this.datatwo,
                        z: 5
                    }
                ]
            };
            let myChartTwo = this.$echarts.init(document.getElementById("two")); //图标初始化\
            myChartTwo.setOption(option);
            window.onresize = function () {
                myChartTwo.resize();
            };
            // 监听容器大小
            window.addEventListener('resize', function () {
                // 调用 resize 和 setOption 方法重新渲染图表
                myChartTwo.resize();
                myChartTwo.setOption(option);
            });
            /*setInterval(function () {
                const dynamicData = [1500, 1200, 1000, 900];
                myChartOne.setOption({
                    series: [
                        {
                            data: dynamicData.slice()
                        },
                        {
                            data: dynamicData.slice()
                        }
                    ]
                });
            }, 3000);*/
        },
        initChartThree(){
            const option = {
                title: {
                    text: '平台作品统计'
                },
                tooltip: {
                    trigger: 'axis',
                    axisPointer: {
                        type: 'shadow'
                    }
                },
                legend: {},
                grid: {
                    left: '3%',
                    right: '4%',
                    bottom: '3%',
                    containLabel: true
                },
                xAxis: {
                    type: 'value',
                    boundaryGap: [0, 0.01]
                },
                yAxis: {
                    type: 'category',
                    data: this.year1
                },
                series: [
                    {
                        name: '图片',
                        type: 'bar',
                        data: this.datathreei
                    },
                    {
                        name: '视频',
                        type: 'bar',
                        data: this.datathreev
                    },
                    {
                        name: '音频',
                        type: 'bar',
                        data: this.datathreea
                    },
                ]
            };
            let myChartThree = this.$echarts.init(document.getElementById("three")); //图标初始化\
            myChartThree.setOption(option);
            window.onresize = function () {
                myChartThree.resize();
            };
            window.addEventListener('resize', function () {
                // 调用 resize 和 setOption 方法重新渲染图表
                myChartThree.resize();
                myChartThree.setOption(option);
            });
        },
        initChartFour(){
            const option = {
                title: {
                    text: '非遗项目类型分布情况',
                    left: 'center'
                },
                tooltip: {
                    trigger: 'item',
                    formatter: '{a} <br/>{b} : {c} ({d}%)'
                },
                legend: {
                    orient: 'vertical',
                    left: 'left'
                },
                series: [
                    {
                        name: 'Access From',
                        type: 'pie',
                        radius: '50%',
                        data: this.datafour,
                        emphasis: {
                            itemStyle: {
                                shadowBlur: 10,
                                shadowOffsetX: 0,
                                shadowColor: 'rgba(0, 0, 0, 0.5)'
                            }
                        }
                    }
                ]
            };
            let initChartFour = this.$echarts.init(document.getElementById("four")); //图标初始化\
            initChartFour.setOption(option);
            window.onresize = function () {
                initChartFour.resize();
            };
            window.addEventListener('resize', function () {
                // 调用 resize 和 setOption 方法重新渲染图表
                initChartFour.resize();
                initChartFour.setOption(option);
            });
        },
        count(){
            this.$http.get('http://localhost:8083/count/zuopin')
                .then(res=>{
                    this.count1 = res.data.count1;
                    this.count2 = res.data.count2;
                    this.count3 = res.data.count3;
                    this.count4 = res.data.count4;
                    this.datatwo = res.data.datatwo;
                    this.year = res.data.year;
                    this.datathreei = res.data.datathreei;
                    console.log(this.datathreei)
                    this.datathreev = res.data.datathreev
                    console.log(this.datathreev)
                    this.datathreea = res.data.datathreea
                    this.year1 = res.data.year1
                    this.name = res.data.name
                    this.value = res.data.count
                    console.log("输出",this.name)
                    for(let i = 0; i<this.value.length; i++)
                    {
                        this.seriesData.push({
                            name:this.name[i],
                            value:this.value[i]

                        });
                        this.legendData.push(this.name[i]);
                    }
                    this.namefour = res.data.namefour
                    this.valuefour = res.data.valuefour
                    for(let i = 0; i<this.namefour.length; i++)
                    {
                        this.datafour.push({
                            name:this.namefour[i],
                            value:this.valuefour[i]
                        });
                    }

                    console.log(this.datafour)


                })
        }
    },

}
</script>

<style scoped>



.responsive-image {
    width: 100%;

}

/* .body{
     !*height: calc(100vh - 100px); !* 让内容区域占据整个视口高度减去头部高度的空间 *!*!
     !*overflow-y: scroll; !* 允许出现垂直滚动条 *!*!
     margin-top: 30px;
 }*/
/*  ::-webkit-scrollbar {
      display: none;
  }*/
.responsive-image img {
    height: 100%;
}
.el-row {
    margin-bottom: 20px;
    &:last-child {
        margin-bottom: 0;
    }
}
.el-col {
    border-radius: 4px;
}
.bg-purple-dark {
    background: #99a9bf;
}
.bg-purple {
    background: #d3dce6;
}
.bg-purple-light {
    background: #e5e9f2;
}
.grid-content {
    border-radius: 4px;
    min-height: 36px;
}
.row-bg {
    padding: 10px 0;
    background-color: #f9fafc;
}

.el-card{
    height: 250px;
}
img{
    width: 150px;
    height: 150px;
}

h1{
    font-size: 25px;
}

#two{
    height: 200px;
    /*width: 500px;*/
    width: 100%;
}
#one{
    margin-top: 30px;
    height: 350px;
    /*width: 600px;*/
    width: 100%;
}
#three{
    margin-top: 40px;
    height: 450px;
    /*width: 500px;*/
    width: 100%;
}
#four{
    height: 350px;
    /*width: 500px;*/
    width: 100%;
}
</style>